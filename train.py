...
def parse_args():
    ...
    parser.add_argument('--eval',default=True, help="If set, then only evaluation will be done")
    parser.add_argument('--eval_dataset', type=str, default='val', help="Dataset split on which evaluate. Can be 'train' and 'val'")

    parser.add_argument("--local_rank", type=int, help="Local rank of the process on the node")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility")

    parser.add_argument("--logdir", type=str, default="/home/zjlab/lizao/test_dir", help="Path, where logs will be stored")
...
def one_epoch(model, criterion, opt, config, dataloader, device, epoch, n_iters_total=0, is_train=True, caption='', master=False, experiment_dir=None, writer=None):
    ...
    save_evl_3d = defaultdict(list)
    save_gt_3d = defaultdict(list)
    name_last = []
    ...
    with grad_context():
        ...
        for iter_i, batch in iterator:
            with autograd.detect_anomaly():
                ...
                if model_type == "alg" or model_type == "ransac":
                    keypoints_3d_pred, keypoints_2d_pred, heatmaps_pred, confidences_pred,_,_ = model(images_batch,
                ...
                # inp = batch['detections'][0]
                # key_2ds = keypoints_2d_pred[0]
                ...
                if iter_i == 0 or str(batch['actname'])!=str(name_last):    
                    keypoints_3d = keypoints_3d_pred
                    keypoints_gt = keypoints_3d_gt

                else:
                    keypoints_3d = torch.cat((keypoints_3d, keypoints_3d_pred), dim=0)
                    keypoints_gt = torch.cat((keypoints_gt, keypoints_3d_gt), dim=0)

                save_evl_3d[''.join(batch['actname'])] = keypoints_3d.cpu().numpy()
                print(''.join(batch['actname']))
                print(save_evl_3d[''.join(batch['actname'])].shape)

                save_gt_3d[''.join(batch['actname'])] = keypoints_gt.cpu().numpy()
                print(save_gt_3d[''.join(batch['actname'])].shape)   
                name_last = batch['actname']
                ...
                if master:
                    ...
                    # dump to tensorboard per-iter loss/metric stats /home/zjlab/lizao/human3.6-small
                    if is_train:
        ...
        np.save('/home/zjlab/lizao/real-time-pose-estimation/val_3d_output.npy', save_evl_3d, allow_pickle=True)
        np.save('/home/zjlab/lizao/real-time-pose-estimation/val_3d_gt.npy', save_gt_3d, allow_pickle=True)
