...
class RANSACTriangulationNet(nn.Module):
    ...
    def forward(self, images, proj_matricies, batch):
        ...
        # print('-------------- detect 2D kpts spends {:.2f} seconds'.format(time1))
        ...
        # print('-------------- detect 3D kpts spends {:.2f} seconds'.format(time2))
...
class AlgebraicTriangulationNet(nn.Module):
    def __init__(self, config, device='cuda:0'):
        ...
        # self.backbone = pose_resnet.get_pose_net(config.model.backbone, device=device)
        self.use_tensorRT = config.tensorRT.use_tensorRT
        if config.tensorRT.use_tensorRT:

            # self.TRT_model = TRT_model(config.tensorRT.engine_2d)
            self.TRT_model = TrtModel_yolo(config.tensorRT.engine_2d)
        else:
            self.backbone = pose_resnet.get_pose_net(config.model.backbone, device=device)
    ...
    def forward(self, images, proj_matricies, batch):
        ...
        if self.use_confidences:
            if self.use_tensorRT:
                # trt_out = self.TRT_model(np.array(images.cpu()).astype(np.float32), batch_size)
                # heatmaps = torch.from_numpy(trt_out[2].reshape(n_views, -1, 96, 96)).to(device)
                # # print(heatmaps.device)
                # alg_confidences = torch.from_numpy(trt_out[0].reshape(n_views, -1)).to(device)

                trt_out = self.TRT_model(images)

                heatmaps = trt_out[2].reshape(n_views, -1, 96, 96)
                alg_confidences = trt_out[0].reshape(n_views, -1)
            else:
        ...
        return keypoints_3d, keypoints_2d, heatmaps, alg_confidences, t1, t2
...
class VolumetricTriangulationNet(nn.Module):
    def __init__(self, config, device='cuda:0'):
        ...
        # transfer
        self.transfer_cmu_to_human36m = config.model.transfer_cmu_to_human36m if hasattr(config.model,
    ...
    def forward(self, images, proj_matricies, batch):
        ...
        proj_matricies = torch.stack(
            [torch.stack([torch.from_numpy(camera.projection) for camera in camera_batch], dim=0) for camera_batch in
             new_cameras], dim=0).transpose(1, 0)  # shape (batch_size, n_views, 3, 4)
        ...
        vol_keypoints_3d, volumes = op.integrate_tensor_3d_with_coordinates(volumes * self.volume_multiplier,
                                                                            coord_volumes, softmax=self.volume_softmax)
        # time2 = time.time() - time0 - time1
        # print('-------------- detect 3D kpts spends {:.2f} seconds'.format(time2))
