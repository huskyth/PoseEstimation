...
def make_collate_fn(randomize_n_views=True, min_n_views=10, max_n_views=31):
    ...
    def collate_fn(items):
        ...
        batch['actname'] = [item['actname'] for item in items]
...
def make_collate_fn_test(randomize_n_views=True, min_n_views=10, max_n_views=31):
...
def prepare_batch_test(batch, device, config, is_train=True):
    # images
    images_batch = []
    for image_batch in batch['images']:
        image_batch = image_batch_to_torch(image_batch)
...
def prepare_batch_video(sample, views, device):
    # images
    batchsize = 1
    images_batch = [[] * batchsize]
    i = 0

    for index, image_batch in enumerate(sample['inps']):
        if index % views == 0 and index != 0:
            i += 1
        images_batch[i].append(image_batch)
    for j in range(len(images_batch)):
        images_batch[j] = torch.stack(images_batch[j], dim=0)
    images_batch = torch.stack(images_batch, dim=0).to(device)

    # for camera_batch in sample['cameras']:
    #     camera = torch.from_numpy(camera_batch.projection)
    #     proj_matricies_batch = torch.stack(camera, dim=0)
    proj_matricies_batch = torch.stack([torch.stack([torch.from_numpy(camera_batch.projection)], dim=0) for camera_batch in sample['cameras']], dim=0).transpose(1, 0)

        # projection matricies
    # proj_matricies_batch = torch.stack([torch.stack([torch.from_numpy(camera.projection) for camera in camera_batch], dim=0) for camera_batch in sample['cameras']], dim=0).transpose(1, 0)  # shape (batch_size, n_views, 3, 4)
    proj_matricies_batch = proj_matricies_batch.float().to(device)
