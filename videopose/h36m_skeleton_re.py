...
class H36mSkeleton(object):
    ...
    def get_bvh_header(self, poses_3d):
        ...
        return header, initial_offset


    def pose2euler(self, pose, header, pose_init):
        channel = []
        quats = {}
        eulers = {}
        stack = [header.root]
        ...
        #     pose_new.append(pose[j] - pose[index['Hip']] +
        #                     np.array([-0.00003500832463032566, 1.0874850749969483, -0.004417469725012779]))
        # pose = np.array(pose_new)
        bone_len = np.linalg.norm(pose_init[index['RightKnee']] - pose_init[index['RightAnkle']])

        # unity_len = 0.5663775315477128 # hip到knee的长度
        unity_len = 0.43
        scale = unity_len / bone_len
        # dist = pose_init[index['Hip']] * scale - np.array([-0.0000368508699, 1.14472115, -0.00464996835])
        for j in range(len(pose)):
            pose_new.append(pose[j] * scale)
        pose = np.array(pose_new)
        while stack:
            node = stack.pop()
            joint = node.name
            ...
            if joint == 'Hip':
                ...
                x_dir = pose[index['LeftHip']] - pose[joint_idx]
                y_dir = None
                z_dir = pose[index['Spine']] - pose[joint_idx]
                order = 'zyx'
            elif joint in ['RightHip']:
                ...
                order = 'zyx'
            elif joint in ['RightKnee']:
                child_idx = self.keypoint2index[node.children[0].name]
                x_dir = pose[index['Hip']] - pose[index['RightHip']]
                y_dir = None
                z_dir = pose[joint_idx] - pose[child_idx]
                order = 'zyx'
            elif joint in ['LeftHip']:
                child_idx = self.keypoint2index[node.children[0].name]
                x_dir = pose[index['LeftHip']] - pose[index['Hip']]
                y_dir = None
                z_dir = (pose[joint_idx] - pose[child_idx])
                order = 'zyx'
            elif joint in ['LeftKnee']:
                child_idx = self.keypoint2index[node.children[0].name]
                x_dir = pose[index['LeftHip']] - pose[index['Hip']]
                y_dir = None
                z_dir = pose[joint_idx] - pose[child_idx]
    ...
    def poses2bvh(self, poses_3d, header=None, output_file=None):
        if not header:
            header, initial_offset = self.get_bvh_header(poses_3d)

        channels = []
        poses = []
        for frame, pose in enumerate(poses_3d):
            channel, pose = self.pose2euler(pose, header, poses_3d[0])
