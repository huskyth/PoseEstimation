...
from videopose.yolo.utils import TryExcept, emojis
...
def _compute_iou(bboxA, bboxB):
    ...
    if union_area == 0:
        union_area = 1e-5
        LOGGER.warning('union_area=0 is unexpected')
...
# def Adjust_boxs(boxs, points):
#     for i, box in enumerate(boxs):
#         # H_box = torch.abs(box[0,3]-box[0,1])
#         H_point_max = points[0, i, 16,1]
#         H_point_min = points[0, i, 9,1]
#         dis0=H_point_max-box[0, 1]
#         dis1=H_point_max-H_point_min
#         print(H_point_max-box[0, 1], H_point_max-H_point_min, H_point_min-box[0, 1])
#         # if H_box > (H_point_max-H_point_min)*1.2:
#         #     boxs[i, 0, 3] = H_point_max + (H_point_max-H_point_min)*0.1
#     return boxs
    

import copy
def track_box(boxs, box_last):
    print("boxs:", len(boxs), boxs[0].shape, boxs[1].shape)
    # print(len(box_last), box_last[0].shape)
    max_iou_score = -1
    max_index = -1
    track_result = 0
    box_out = copy.deepcopy(box_last)

    for i in range(len(boxs)):
        boxB = list(box_last[i][0])
        for j in range(boxs[i].shape[0]):
            boxA = list(boxs[i][j])
            iou_score = _compute_iou(boxA, boxB)
            if iou_score > max_iou_score:
                max_iou_score = iou_score
                max_index = j
        # print(boxs[i][max_index].shape)
        if max_iou_score > 0.1:
            box_out[i][0] = boxs[i][max_index]
            max_iou_score = -1
            max_index = -1
            track_result = 1
        else:
            print("stop track")
            track_result = 0
            return box_out, track_result
    # print(len(box_last), box_last[0].shape)
    return box_out, track_result


def track_boxcolor(boxs, box_last, color_begin, img, last_img):
    track_index = []
    save_iou = []
    track_result = 0
    box_out = copy.deepcopy(box_last)
    ...
    for i in range(len(boxs)):
        ...
        for j in range(boxs[i].shape[0]):
            ...
            iou_score = _compute_iou(boxA, boxB)
            if iou_score > 0.2:
                track_index.append(boxs[i][j])
                save_iou.append(iou_score)
                # track_index[num_track] = j
                # num_track += 1
        if len(track_index)==0:
        ...
        elif len(track_index)==1:
            box_out[i][0] = track_index[0]
            track_result = 1
            track_index.clear()
            save_iou.clear()
            # num_track=0
            ...
            iou_index = 0
            sec_index = 0
            for l, iou in enumerate(save_iou):
                if iou > max_iou:
                    max_iou = iou
                    iou_index = l
            for l, iou in enumerate(save_iou):
                if iou < max_iou and iou > sec_iou:
                    sec_iou = iou
                    sec_index = l

            if max_iou - sec_iou > 0.3:
                track_id = iou_index
            else:
                print(iou_index, max_iou)
                print(sec_index, sec_iou)
                if i == 0:
                    F_cam = 1
                elif i==1:
                    F_cam = 0
                    ...
                    F_cam = 2
                F_last = fast_color_histogram(last_img[F_cam], box_last[F_cam][0])
                if F_last is not None:
                    track_id = compare_img(img[i], track_index, F_last)
                else:
                    track_id = -1
                if track_id == -1:
                    track_id = iou_index
            box_out[i][0] = track_index[track_id]
            track_result = 1
            track_index.clear()
            save_iou.clear()
    return box_out, track_result

def compare_img(img, bboxs, F_begin):
    max_F = 0
    sec_F = 0
    track_id = -1
    # def BC
    for k, bbox in enumerate(bboxs):
        H1 = fast_color_histogram(img, bbox)
        ## 计算suft相似度
        if H1 is not None:
            F_sim = cal_SIFT_sim(H1, F_begin)
            # F_sim = cv2.compareHist(H1, F_begin, cv2.HISTCMP_CORREL)  # 计算hist相似度
            if max_F < F_sim:
                max_F = F_sim
                track_id = k
            elif F_sim == max_F:
                sec_F = F_sim
            print("box", k, "simliar:", F_sim)
        if max_F==sec_F:
            track_id = -1
    return track_id


def fast_color_histogram(image, bbox, color_stride=384):
    # input:image is a 3d array, shape[W][H][C]
    x1, y1, x2, y2, conf = bbox[:5]
    orig_h, orig_w = image.shape[0], image.shape[1]
    ratio_coeff = orig_h / color_stride
    x1 = x1 * ratio_coeff
    x2 = x2 * ratio_coeff
    ...
    # 计算surf
    surf = cv2.ORB.create(100)
    kp, des = surf.detectAndCompute(img, None)
    # bg = cv2.drawKeypoints(img, kp, None, (0, 255, 0), 4)
    # bg = cv2.resize(bg, (100, 200))
    # cv2.imshow(str(11), bg)
    return des
...
def cal_SIFT_sim(descriptors1,descriptors2):
    bf = cv2.BFMatcher()
