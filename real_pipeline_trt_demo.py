...
from send_msg import StartTCP, SendMsg, SendVideo
...
def det_preproc(streams, inp_dim, msg):
    ...
    for v in range(len(streams)):
        ...
        if send_video:
            if send_video.flag == 0:
                videos_to_save[v].append(frame)
            elif send_video.flag == 1:
                send_video.save(videos_to_save)
...
if __name__ == '__main__':
    ...
    path = '/home/zjlab/real-time-pose-estimation/system/video/'
    os.makedirs(temp_path, exist_ok=True)
    os.makedirs(path, exist_ok=True)
    frames_to_save = defaultdict(list)
    videos_to_save = defaultdict(list)
    ...
    send_video = SendVideo(path).start()
